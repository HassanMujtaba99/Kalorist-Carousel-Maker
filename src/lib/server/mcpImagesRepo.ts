...
export async function saveMcpImage(userId: string, dataUrl: string): Promise<string> {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      await sql()`
        INSERT INTO mcp_images (tag, user_id, mime_type, data_base64, created_at)
        VALUES (${tag}, ${userId}, ${parsed.mimeType}, ${parsed.base64}, ${Date.now()})
      `;
      return tag;
    } catch (e) {
...
export async function listMcpImages(userId: string): Promise<McpImageSummary[]> {
  await ensureSchema();
  const rows = (await sql()`
    SELECT tag, created_at FROM mcp_images WHERE user_id = ${userId} ORDER BY created_at DESC LIMIT 50
  `) as { tag: string; created_at: number }[];
  return rows.map((r) => ({ tag: r.tag, createdAt: Number(r.created_at) }));
}
